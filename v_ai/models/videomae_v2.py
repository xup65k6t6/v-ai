...
class VideoMAEV2ClassificationModel(nn.Module):
    def __init__(self, num_frames=8, image_size= 224,pretrained=True, model_name="OpenGVLab/VideoMAEv2-Base"):
        ...
        if pretrained:
            ...
            self.videomae = VideoMAEForVideoClassification.from_pretrained(model_name, config=config, ignore_mismatched_sizes=True, attn_implementation="sdpa", torch_dtype= (torch.bfloat16 if torch.cuda.is_available() else torch.float32))
        else:
